...
```sql
-- who missed yesterday's session and has a phone number
select id, first_name as name, mobile as phone_number, email
from contacts
where session_status = 'no_show' and session_date = current_date - 1 and mobile is not null;
```

```bash
# daily 10:00 via cron / GitHub Actions / Cloud Scheduler
psql "$DATABASE_URL" -At -F$'\t' -c "$QUERY" \
 | jq -Rn '{campaign_key:"no_show_recovery",recipients:[inputs|split("\t")|{id:.[0],name:.[1],phone_number:.[2],email:.[3]}]}' \
